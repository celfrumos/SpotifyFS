...
//type HttpProcessor(srv: HttpServer) =
...
//    member this.Process(socket: TcpClient) =
...
//    member this.ParseRequest(request: string) =
...
//    member this.ReadHeaders(requestLines: seq<string>) =
...
//    member this.HandleGetRequest() =        
//        _srv.HandleGetRequest(this)
        

//    member this.HandlePostRequest()=
...
//and [<AbstractClass>]HttpServer(Port:int) = class
...
//    abstract member HandleGetRequest: HttpProcessor -> unit

//    abstract member HandlePostRequest: HttpProcessor * StreamReader-> unit
//    member this.Listen() =            
//        try                
...
//    override this.HandleGetRequest(p) =
        
//            p.WriteSuccess()
//            if p.HttpUrl = "/favicon.ico" then
//                ()
//            else
...
//                    let url = p.HttpUrl
//                    let url = url.Substring(2, url.Length - 2)
...
//                        t <- new Thread(new ThreadStart(fun _ -> _onAuth.Trigger(new AuthEventArgs(col.Get(1), col.Get(0)))))
                    
//                    else
                    
//                        p.OutputStream.WriteLine("<html><body><h1>Spotify Auth successful!</h1><script>window.close();</script></body></html>")
//                        t <- new Thread(new ThreadStart(fun o -> _onAuth.Trigger(new AuthEventArgs(col.Get(0), col.Get(1)))))
...
//                            p.OutputStream.WriteLine("<html><body><h1>Spotify Auth successful!</h1><script>window.close();</script></body></html>");
//                            t <- new Thread(new ThreadStart(fun o -> _onAuth.Trigger(new AuthEventArgs(col.Get(0), col.Get(1), col.Get(3), expiresIn = Convert.ToDouble(col.Get(2))))))
//                            p.OutputStream.Flush()
//                            p.OutputStream.Close()
//                if t = null 
//                then () 
