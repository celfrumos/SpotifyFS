namespace SpotifyFS
    ...
    type SpotifyWebApi(auth: SpotifyAuthResponse, ?apiConfig) =  class
        ...
        member this.DownloadDataAsync(url) (*where 'T : BasicModel *) = async { 
            ...
            let rec doDownload first (triesLeft: int) (lastError: Error option) (response: (ResponseInfo * string) option) = async {
                ...
                if first || this.UseAutoRetry && triesLeft > 0 && (lastError.IsNone || hasRetryErrorCode()) then
                    ...
                    let! (inf, item) as res = this.DownloadDataAltAsync(url)
                    printfn "%A\n\n%s" inf item
                    return! doDownload false (triesLeft - 1) (inf.Error) (Some res)                
                    
                else
                    return response
            }
            return! doDownload true this.RetryTimes None None            
         }
...
//        }
        ...
        /// <param name="market">An ISO 3166-1 alpha-2 country code. Provide this parameter if you want to apply Track Relinking.</param>
        /// <returns></returns>
        member this.GetAlbumAsync(id, ?market) = async {
